...
export function DeepRelax({ onBack }) {
  const {
    isRunning,
    currentPhase,
    phaseLabel,
    ...
    pause,
    reset,
  } = useBreathTimer(DEEP_RELAX_PATTERN)
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <h2 className="text-2xl font-light mb-2">Deep Relaxation</h2>
      <p className="text-slate-400 text-sm mb-8">Extended exhale for deep calm</p>
      ...
      <div className="mt-12 p-6 rounded-2xl bg-white/5 border border-white/10 max-w-sm">
        ...
        <div className="grid grid-cols-3 gap-2 text-center text-xs">
          ...
          <div className="p-2 rounded-lg bg-emerald-500/20 text-emerald-300">
            ...
            <p className="text-lg mt-1">8s</p>
          </div>
        </div>
        <p className="text-slate-500 text-xs mt-3 text-center">2x exhale length maximizes parasympathetic activation</p>
      </div>
    </div>
  )
}
